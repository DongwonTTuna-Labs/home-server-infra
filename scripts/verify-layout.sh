...
required=(
  ...
  dotfiles/README.md
  dotfiles/bin/build-agent-docs.py
  dotfiles/bin/install-agent-config.sh
  dotfiles/agent-rules/00-core.md
  dotfiles/agent-rules/10-engineering.md
...
# The per-tool agent files are generated from dotfiles/agent-rules/. Regenerate
# and diff so a hand-edit of AGENTS.md or CLAUDE.md fails here instead of being
# silently overwritten by the next install.
python3 dotfiles/bin/build-agent-docs.py --check

for generated in dotfiles/codex/AGENTS.md dotfiles/claude/CLAUDE.md; do
  if ! head -1 "$generated" | grep -Fq 'Generated by dotfiles/bin/build-agent-docs.py'; then
    printf 'Generated agent file lost its do-not-edit banner: %s\n' "$generated" >&2
    exit 1
  fi
done

# Layer 1 stays tool-wide. A project contract in the always-loaded file is what
# made the previous global AGENTS.md 40% about a single stack. This is a
# tripwire, not a proof: a project contract always names the paths it governs,
# so reject repository and working-directory paths here. Naming a project as
# the source of a decision is fine, and the gpt-webai-pro delegation rule is
# deliberately in the canon because it is how any session reaches ChatGPT.
if grep -En 'stacks/|~/Documents/|docs/plan/' dotfiles/agent-rules/*.md; then
  printf 'Shared agent canon must not carry project-specific contracts\n' >&2
  exit 1
fi
