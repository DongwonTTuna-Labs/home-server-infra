...
전송 직후 연결이 끊기면 이미지 도구의 공백 렌더링과 접기 버튼을 전송 확인과 같은
방식으로 처리해 기존 턴을 찾습니다. 다운로드도 매 원본 선택 직전에 예상 수량과
이미지 로딩을 재검사합니다. 첫 썸네일이 빠져 순번이 밀린 갤러리는 저장하지 않으며,
원래 갤러리가 복구된 뒤 같은 요청을 재개할 수 있습니다.

보정할 PNG와 제작 맥락 ZIP은 `--file`로 함께 첨부할 수 있습니다. PNG 타일은 본문
텍스트 대신 파일명을 가진 접근성 group으로 확인하며, 파일 개수·이름·업로드 상태와
이미지 디코딩 준비를 모두 검사한 뒤 전송합니다. 웹 UI의 `파일명(1).zip` 재명명도
인식합니다. 첨부 불일치는 클릭 전에 차단하고 해당 화면의 진단 경로를 오류에 남깁니다.
...
전송 직후 임시 대화 주소가 바뀌면 확인된 사용자 메시지가 있는 기존 탭을 찾습니다. 해당 메시지를 확인할 수 없는 빈 화면은 진단 성공으로 저장하지 않습니다.
