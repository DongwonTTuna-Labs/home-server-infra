...
출력합니다.
`cleanup`은 기본이 dry-run이며, `--apply`일 때만 오래된 고아 컨테이너 정지와
`needs_login` 재검사를 실제로 처리합니다. `release`는 요청을 `failed`로 강제 종결하고 더
이상 active 요청이 없는 슬롯 컨테이너를 정지합니다.
...
기존 DB 상태를 보존합니다. 이미 비종결 요청이 있는 슬롯은 살아 있으므로 건너뛰며, 원래
꺼져 있던 컨테이너는 검사 후 다시 정지합니다. `state`는 최종 DB 상태, `probe`는 이번 관찰
...
매일 자동 실행할 user timer를 설치합니다. timer는 09:20 이후 최대 10분의 무작위 지연을
두며, 놓친 실행은 user manager가 다시 시작될 때 보충합니다.
...
systemctl --user enable --now gpt-webai-pro-keepalive.timer
systemctl --user start gpt-webai-pro-keepalive.service  # 선택적 즉시 점검
systemctl --user list-timers gpt-webai-pro-keepalive.timer
```
