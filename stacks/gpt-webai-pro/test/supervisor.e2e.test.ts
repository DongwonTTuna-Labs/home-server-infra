...
test("reap advances one abandoned request per tick, rotates fairly, and ignores staged sends", async (t) => {
  ...
  const secondAbandoned = await supervisor.run("second abandoned owner", [], 0);
  assert.equal(secondAbandoned.status, "running");
  ...
  const rotated = await supervisor.reap(0);
  assert.deepEqual(
    rotated.actions.map((action) => [action.session, action.before, action.after]),
    [[secondAbandoned.sessionId, "generating", "running"]],
  );
...
test("a running envelope releases an ownerless managed runtime without terminalizing the request", async (t) => {
  const { supervisor, daemons } = await fixture(t, [{
    id: "slot-a",
    account: "a",
    handler: (method, params) => {
      if (method === "readiness") return { state: "ready", modelLabel: "Pro" };
      if (method === "send") return {
        conversationUrl: "https://chatgpt.com/c/runtime-lease",
        userTurnId: "user-runtime-lease",
        assistantTurnId: "assistant-runtime-lease",
      };
      if (method === "poll") return { state: "generating", currentUrl: pollUrl(params) };
      throw new Error(`unexpected ${method}`);
    },
  }]);
  ...
  const first = await supervisor.run("release runtime lease", [], 0);
  assert.equal(first.status, "running");
  assert.equal(supervisor.db.getRequest(first.sessionId!)?.status, "generating");
  assert.equal(running, false);
  assert.equal(stopCalls, 1);
  const resumed = await supervisor.resume(first.sessionId!, 0);
