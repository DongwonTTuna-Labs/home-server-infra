...
export async function acquireFileLock(
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs < 0) {
    throw new Error("file lock timeout must be a non-negative number");
  }
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    const lock = await tryAcquireFileLock(filename);
    if (lock) return lock;
    if (Date.now() >= deadline) throw new Error(`timed out acquiring file lock: ${filename}`);
