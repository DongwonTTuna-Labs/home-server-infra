...
curl, and jq. GitHub workflows should use the `dongwontuna-labs-runner` label.
...
Codex review workflows authenticate through the GitHub organization secret
`AI_RELAY_API_KEY` and the relay provider at
`https://relay-ai.dongwontuna.net/v1`. The runner containers do not persist
per-runner Codex login state.

Create or update the org-wide secret with a GitHub credential that can manage
organization Actions secrets:

```bash
gh secret set AI_RELAY_API_KEY --org DongwonTTuna-Labs --visibility all
```
