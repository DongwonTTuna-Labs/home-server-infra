...
def test_no_inline_python_or_schema_bloat():
    ...
    # The helper is installed as an editable package from the trusted core, then
    # invoked as a console script (not a vendored bin path, not the PR-head tree).
    assert "pip install --disable-pip-version-check -e trusted-core/setup/codex-review" in text
